...
//csqla:="UPDATE SF1010 SET F1_XNOMPRO=A2_NOME FROM SF1010 SF1 LEFT JOIN SA2010 SA2 ON F1_FORNECE=A2_COD AND F1_LOJA=A2_LOJA AND SA2.D_E_L_E_T_='' AND A2_FILIAL='' WHERE F1_FILIAL='01' AND SF1.D_E_L_E_T_='' AND F1_ESPECIE IN ('NF ','RCN')"
				
//lResult := TcSqlExec(csqla)
//                If lResult < 0
//                    alert(TCSQLError())
//                EndIf

DEFINE MSDIALOG _oDlg TITLE OemtoAnsi("Trnasferencia Arquivos") FROM C(178),C(181) TO C(548),C(717) PIXEL
...
Else
   IF Len( aArquivos ) > 0
        ...
        For nIxb := 1 To Len( aArquivos )
		    IncProc()
		    IF nRadioGrp1 == 1  // Enviar 
               lCopiou :=  CpyT2S(cDirectory+aArquivos[nIxb],cDirDestino)
...
If Len( aRetSel ) == 0
	Aviso("Atencao","Nenhum arquivo foi selecionado para transferencia",{"Ok"},2,"Trnasferencia")
Endif
