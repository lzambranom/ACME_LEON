...
+===========================================================================+
| Desc.     #  Funci�n para crear columnas de SEMAFORIZACION                |
|           #                                                               |
+===========================================================================+
| Uso       # 	@example                                                    |
|           #   u_ACM001()                                                |
...
USER function ACM001()
	...
	chkfile("ZZU")
	/*
	AAdd( aCampos,{ "ZX2_DOC"	,"N�mero Salida"	,01,"@!",1,TamSX3("ZX2_DOC")[1]		,0,.T.	} )
	AAdd( aCampos,{ "ZX2_SERIE"	,"Serie Salida"		,02,"@!",1,TamSX3("ZX2_SERIE")[1]	,0,.T.	} )
	AAdd( aCampos,{ "ZX2_DOC2"	,"N�mero Entrada"	,01,"@!",1,TamSX3("ZX2_DOC2")[1]	,0,.T.	} )
	...
	AAdd( aCampos,{ "ZX2_DTDIGI","Fecha Digitaci�n"	,04,"@!",1,TamSX3("ZX2_DTDIGI")[1]	,0,.T.	} )
	AAdd( aCampos,{ "ZX2_ALMORI","Almac�n Origen  "	,05,"@!",1,TamSX3("ZX2_ALMORI")[1]	,0,.T.	} )
	AAdd( aCampos,{ "ZX2_ALMDES","Almac�n Destino"	,06,"@!",1,TamSX3("ZX2_ALMDES")[1]	,0,.T.	} )
	AAdd( aCampos,{ "ZX2_OBSER"	,"Observaci�n "		,07,"@!",1,TamSX3("ZX2_OBSER")[1]	,0,.T.	} )
	*/
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Axel Diaz                                                    |
 | Data:  24/12/2019                                                   |
 | Desc:  Creaci�n de Menu del modelo de datos MVC                     |
 *---------------------------------------------------------------------*/
Static Function MenuDef()
	...
	ADD OPTION aRot TITLE 'Imprimir Legalizadci�n' 				ACTION 'processa( {|| U_ACMR01A("3",cPath) }, "Preparando Archivo", "Leyendo registros...", .f.)' 	OPERATION 9 					 ACCESS 0 //OPERATION 9
...
/*---------------------------------------------------------------------*
 ...
 | Autor: Axel Diaz                                                    |
 | Data:  24/12/2019                                                   |
 | Desc:  Creaci�n de modelo de datos MVC                              |
 *---------------------------------------------------------------------*/
Static Function ModelDef()
    ...
    For nAtual := 1 To Len(aTriggeHD)
        oStHeader:AddTrigger(   aTriggeHD[nAtual][01],; //Campo Origen
                            	...
                            	aTriggeHD[nAtual][03],; //Bloque de c�digo de validacion que ejecutara el gatillo
                            	aTriggeHD[nAtual][04])  //Bloque de c�digo de execucion 
    Next
	...
	oModel:GetModel('ZY2DETAIL'):SetUniqueLine({"ZY2_CODPRO"})	// No repita informaci�n o combinaciones {"CAMPO1", "CAMPO2", "CAMPOX"}
	oModel:SetPrimaryKey({})
	
	// Establecer las descripciones
	oModel:SetDescription("Semaforizaci�n Acme Leon")
...
Static Function GMSDetail()
	...
	aCampo:=u_SX3Datos('ZY2_DTDIG2');	oStruct:AddField(aCampo[6]	,aCampo[7]	,aCampo[2]	,aCampo[3]	,aCampo[4]		, aCampo[5]	,,,{}, aCampo[19], , .F., .F., .F., , )
										...
										oStruct:AddField('Descrip'	,'Descripci�n','DESCRIP', 'C'		,TamSX3("B1_DESC")[1], 0	,,,{}, .F.		, FWBuildFeature( STRUCT_FEATURE_INIPAD, "u_MVC011Des()"), .F., .F., .T., , ) 
...
Static Function GVSDetail()
	...
	aCampo:=u_SX3Datos('ZY2_OBSERV');	oStruct:AddField(/*cIdField*/ aCampo[2],/*cOrdem*/ '14',/*cTitulo*/ aCampo[6]	,/*cDescric*/ aCampo[7],/*aHelp*/ {},/*cType*/ aCampo[3],/*cPicture*/ aCampo[8]	,/*bPictVar*/,aCampo[12]/*cLookUp*/, /*lCanChange*/ aCampo[17],/*cFolder*/,/*cGroup*/,/*aComboValues*/,/*nMaxLenCombo*/,/*cIniBrow*/,/*lVirtual*/ .F. ,/*cPictVar*/,/*lInsertLine*/,/*nWidth*/)
										...
										oStruct:AddField(/*cIdField*/ 'DESCRIP',/*cOrdem*/ '04',/*cTitulo*/'Descripci�n', /*cDescric*/ 'Descripci�n del Producto'			,/*aHelp*/,/*cType*/ 'C'  ,/*cPicture*/"@!S45",,,/*lCanChange*/ .F.,,,,,,/*lVirtual*/.T.,, )
...
Static Function PosCommit(oModel) 
		...
		If !(lModiSal .and. cDocModi==cDoc)
			If nOperation==MODEL_OPERATION_INSERT
				If !(cDoc==u_GetSX5Num("ZZ",cSerie,1,cDoc))
					If !U_ValSerDoc(cSerie,cDoc,1)
						...
						Help(NIL, NIL, "N�mero en Uso",NIL,"El n�mero de documento de Salida ya est� en uso", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Se asigno el numero: '+cDoc}) 
					EndIf
				EndIf
				// Actualiza la Serie
				dbSelectArea("SX5")
			...
			Else
				If !U_ValSerDoc(cSerie,cDoc,1)   // Doc existe devuelve falso
					lRet:=.F.
					Help(NIL, NIL, "N�mero en Uso",NIL,"El n�mero de documento de Salida ya est� en uso", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Busque un numero no usado'}) 
					Return lRet
				EndIf
			EndIf
		EndIf
	...
	// -----------------------------------------------------
	// En caso de Insertar nuevo Registro de Entrada o Legalizaci�n
	// -----------------------------------------------------		
	ElseiF nOperation==MODEL_OPERATION_UPDATE // Modificar
		...
		If lLegaliza 
			If !EMPTY(cSD3AcDoc)
				If oModelZX2:GetValue("ZX2_ESTADO")==ESTADO_AMARILLO
		...
		Else
			...
			EndIf
			If !lRet
				Help(NIL, NIL, "Completar Datos",NIL,"Los datos obligatorios no han sido diligenciados", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Revise los campos <br>Serie del documento de Entrada <b>(ZX2_SERIE2)</b>,<br>Almac�n de entrada <b>(ZX2_ALMDES)</b>, <br>Cantidad <b>(ZY2_CANTD)</b>'}) // o <br>Unidad de Medida <b>(ZY2_UNDES)</b>
			Else
				...
				If !U_ValSerDoc(cSerie2,cDoc2,2) .AND.  nTpMov==ESTADO_BLANCO  // Doc existe devuelve falso
					lRet:=.F.
					Help(NIL, NIL, "N�mero en Uso",NIL,"El n�mero de documento de Entrada ya est� en uso", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Busque un numero no usado'}) 
					Return lRet
				Else 
					oModelZY2:GoLine(1)
				...
				//	Help(NIL, NIL, "N�mero de documento Serie 2",NIL,"El n�mero de documento ser� cambiado, dado que el diligenciado "+cDoc+"ya est� siendo utilizado o no cumple con el estandar", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Anote el nuevo n�mero '+cNewDoc})
...
If !EMPTY(cSalFull) .and. lSaldo
	IF cReserv=='3'
		Help(NIL, NIL, "Saldo Comprometido(MV_XACMRSV)",NIL,"Documento grabado, PERO con saldo comprometido SIN Legalizaci�n:(Incluyendo este Despacho)", 1, 0, NIL, NIL, NIL, NIL, NIL,{'Revise:'+cSalFull}) // o <br>Unidad de Medida <b>(ZY2_UNDES)</b>
	ElseIf cReserv="1"
		Help(NIL, NIL, "Saldo Comprometido(MV_XACMRSV)",NIL,"Documento NO podr� grabarse con saldo comprometido sin Legalizaci�n:(Incluyendo este Despacho)", 1, 0, NIL, NIL, NIL, NIL, NIL,{cSalFull}) // o <br>Unidad de Medida <b>(ZY2_UNDES)</b>
	EndIf
EndIf
If lRet
	cDocModi:=""
...
Static Function ACMLPRE( oModelGrid, nLinha, cAcao, cCampo )
	...
	If cAcao == 'DELETE' .AND. nOperation == MODEL_OPERATION_UPDATE
		If !(oModelZY2:GetValue("ZY2_2CANTO")<=0)
			...
			Help( ,, 'Help',, 'No puede eliminar lineas de un tranferencia de entrada' +;
					CRLF + 'Linea N�mero ' + Alltrim( Str( nLinha ) ), 1, 0 )
		EndIf
	EndIf
	...
	If nOperation==MODEL_OPERATION_INSERT .AND. INCLUI
		If Empty(AllTrim(oModelZX2:GetValue('ZX2_SERIE'))) .OR. ;
		   ...
		   MsgInfo('Uno de los campos del encabezado no est� diligenciado.'+CRLF+'Favor revise, Serie/N�mero de documento y Almacen Origen ',"Completar encabezado")
		   //Help( ,, 'Help',, 'Uno de los campos del encabezado no esta diligenciado.' +;
		   //	CRLF + 'Favor revise, Serie o N�mero de documento y Almacen Origen ', 1, 0 )
		   lRet := .F.
		EndIf
...
|           # lSeek	L�gico		Se posicionara ou nao antes da execucao do gatilhos		.F.		                    |
|           # cAlias	Caracteres	Alias da tabela a ser posicionada							X	                |
|           # nOrdem	Num�rico	Ordem da tabela a ser posicionada					0							|
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n derivada FWFormModelStruct del modelo para Colores   |
|           #  de la leyenda                                                |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n derivada FWFormModelStruct del modelo para diferencia|
|           #  de cantidades                                                |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n derivada FWFormModelStruct del modelo para DESCRIOCIO|
|           #  deL PRODUCTO                                                |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n para borrar registo                                  |
|           #                                                               |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n que muestra grupo de preguntas al ingresar           |
|           #                                                               |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n para visuilizas con dobleclic				           |
|           #                                                               |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
...
+---------------------------------------------------------------------------+
| Desc.     #  Funci�n para ingresas salida                                 |
|           #                                                               |
+---------------------------------------------------------------------------+
| Uso       # AP                                                            |
+---------------------------------------------------------------------------+
