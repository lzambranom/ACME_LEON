	...
	If Len(aImpXML) > 0
		For nX :=1 To Len(aImpXML)
			If aImpXML[nX,3] == "D"
				...
				EndIf
				cXML += '		<cbc:PaymentPercent>4</cbc:PaymentPercent>' + cCRLF //Porcentaje de la detracción
	            cXML += '		<cbc:Amount currencyID="PEN">' + alltrim(TRANSFORM(aImpXML[nX,6],"999999999999.99")) + '</cbc:Amount>' + cCRLF //Monto de la Detracción
	      		cXML += '	</cac:PaymentTerms>' + cCRLF	      		
			EndIf
		Next nX
	EndIf
