...
oPrint:Say(399,1310, "C.C. D/H."	,oFont10)
oPrint:Say(399,1570, "Item.D/H."	,oFont10)
//oPrint:Say(399,2060, "Item.C"		,oFont10)
oPrint:Say(399,1760, "VL.D/H."		,oFont10)
//oPrint:Say(399,2400, "VL.C"		,oFont10)
oPrint:Say(399,1960, "Debito."		,oFont10)
oPrint:Say(399,2160, "Credito."		,oFont10)
...
Do While !( (cTablaTemp)->(EOF()) )
	If ((cTablaTemp)->(CT2_LOTE) >= LotIni .AND. (cTablaTemp)->(CT2_LOTE) <= LotFin) 			//Verifica que este en el rango de lotes
		If ALLTRIM((cTablaTemp)->(CT2_SBLOTE)) == SubLt //.OR. _sblot=="000"  					//Verifica el sublote
			If ((cTablaTemp)->(CT2_DOC) >= DctoIni .AND. (cTablaTemp)->(CT2_DOC) <= DctoFin )   //que este dentro de los documentos
				...
				If (cDoc >= DctoIni  .and. cDoc <= DctoFin) .and. (cDoc <> (cTablaTemp)->(CT2_DOC) .OR. nLote <> (cTablaTemp)->(CT2_LOTE) .OR. dFecha <> (cTablaTemp)->(CT2_DATA)) // Si cambia cualquiera cambia de poliza
					...
					oPrint:Say(Linea,1240, RTrim(Transform(docDebe ,"@r 999,999,999.99")),oFont12,,,,1)
					oPrint:Say(Linea,1500, RTrim(Transform(docHaber,"@r 999,999,999.99")),oFont12,,,,1)
					docDebe  := 0
					docHaber := 0
					Linea    := 2500
				Endif
				...
				If AllTrim((cTablaTemp)->(CT2_DC)) <> "4"      
					...
					If AllTrim((cTablaTemp)->(CT2_DC)) == "3" 
				    	oPrint:Say(linea-100,2060, RTrim(Transform(nDebito	,"@r 999,999,999.99"))	,oFont07,,,,1)	//Línea Débito   
				    	oPrint:Say(linea	,2260, RTrim(Transform(nCredito	,"@r 999,999,999.99"))	,oFont07,,,,1)	//Línea Crédito
					Else
				 		If nDebito > 0
				  			oPrint:Say(linea,2060, RTrim(Transform(nDebito,"@r 999,999,999.99"))	,oFont07,,,,1)  //Línea Débito
						Endif
...
oPrint:Say(Linea,1500, RTrim(transform(totalHaber, "@r 999,999,999,999.99")),oFont12,,,,1)
Linea:= Linea + 30	
oPrint:Say(Linea,0060, "-------------------------------------------------------------------------------------------------------------------------------------------------------------------", oFont10)		
Linea:= Linea + 30	
	...
	If TMP->CT2_TPSALD == AllTrim(Str(nTpSld))	//Tipo de Saldo
		...
		If ALLTRIM(TMP->CT2_DC) <> "4"    
			If ALLTRIM(TMP->CT2_DC) == "3"    
			    oPrint:Say(linea-100,2060, RTrim(transform(nDebito,"@r 999,999,999.99")),oFont07,,,,1)  //Linea debito 
			    oPrint:Say(linea,2260, RTrim(transform(nCredito,"@r 999,999,999.99")),oFont07,,,,1)  	//Linea credito	 
		 	Else
				If nDebito > 0
					oPrint:Say(linea,2060, RTrim(transform(nDebito,"@r 999,999,999.99")),oFont07,,,,1)  //Linea debito
				Endif
		...
		If ALLTRIM(TMP->CT2_DC) == "3"  
			...
			//oPrint:Say(linea, 2110, ALLTRIM( SUBSTR((cTablaTemp)->(CTT_DESC01),1,15) ) ,oFont07)      //Descripcion
		   	oPrint:Say(linea, 1570, ALLTRIM(SUBSTR(TMP->CT2_ITEMD,1,5)),oFont07)               			//ItemD  
			//oPrint:Say(linea, 2380, ALLTRIM( SUBSTR((cTablaTemp)->(CTD_DESC01),1,15) ) ,oFont07)      //Descripcion 
		  	oPrint:Say(linea, 1760, ALLTRIM(SUBSTR(TMP->CT2_CLVLDB,1,10)),oFont07)              		//Vld 
			//oPrint:Say(linea, 2850, ALLTRIM( SUBSTR((cTablaTemp)->(CTH_DESC01),1,15) ) ,oFont07)      //Descripcion
			...
			oPrint:Say(linea, 1570, ALLTRIM(SUBSTR(TMP->CT2_ITEMC,1,5)),oFont07)               			//ItemC	
			//oPrint:Say(linea, 2380, ALLTRIM( SUBSTR((cTablaTemp)->(CTD_2_DESC01),1,15) ) ,oFont07)    //Descripcion
		  	oPrint:Say(linea, 1760, ALLTRIM(SUBSTR(TMP->CT2_CLVLCR,1,10)),oFont07)               		//Vlc
			//oPrint:Say(linea, 2850, ALLTRIM( SUBSTR((cTablaTemp)->(CTH_2_DESC01),1,15) ) ,oFont07)    //Descripcion
...
oPrint:Say(Linea,1240, RTrim(Transform(docDebe,  "@r 999,999,999.99")),oFont12,,,,1)
oPrint:Say(Linea,1900, RTrim(Transform(docHaber, "@r 999,999,999.99")),oFont12,,,,1)
Linea:= Linea + 30
oPrint:Say(Linea,0060, "-------------------------------------------------------------------------------------------------------------------------------------------------------------------", oFont10)		
Linea:= Linea + 30
oPrint:Say(Linea,0060, "Total General Informe:",oFont14n)
