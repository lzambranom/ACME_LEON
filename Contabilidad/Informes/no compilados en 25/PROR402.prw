...
DbSelectArea("SX1")
DbSetOrder(1)
DbSeek(Padr("CTR400",Len(X1_GRUPO))+"18")
...
If cpaisLoc =="COL"
	...
	DbSeek(Padr("CTR400",Len(X1_GRUPO))+"19")
	...
	aPergs:={}
	aAdd( aPergs , {"Ordena por ?"	,"¿Ordene por ?" 		,"Order By ?"	,"mv_chp"	,"N"		,1			,			,1	   		,"C"		,""			,"mv_par39","Conta" 		,"Cuenta"			,"Count"			,""				,""				,"N.I.T"		,"N.I.T"		,"N.I.T"	 		,""			,""			,""			,""			,""			,""			,""			,""			,""			,""			,""			,""			,""			,""				,""				,""			,""		,"S"		,""		,""	  		,""		 		,{|| .T.}})
	DbSeek(Padr("CTR400",Len(X1_GRUPO))+"39")
...
//	TRCell():New(oSection1,"CLANCDEB"	,"cArqTmp","DEBITO",/*Picture*/,TAM_VALOR,/*lPixel*/,{|| ValorCTB(cArqTmp->LANCDEB,,,TAM_VALOR,nDecimais,.F.,cPicture,"1",,,,,,lPrintZero,.F.) },/*"RIGHT"*/,,"CENTER",,,.F.)// "DEBITO"
//	TRCell():New(oSection1,"CLANCCRD"	,"cArqTmp","CREDITO",/*Picture*/,TAM_VALOR,/*lPixel*/,{|| ValorCTB(cArqTmp->LANCCRD,,,TAM_VALOR,nDecimais,.F.,cPicture,"2",,,,,,lPrintZero,.F.) },/*"RIGHT"*/,,"CENTER",,,.F.)// "CREDITO"
 //	TRCell():New(oSection1,"CTPSLDATU"	,"cArqTmp","SALDO ACTUAL",/*Picture*/,TAM_VALOR+2,/*lPixel*/,/*{|| }*/,/*"RIGHT"*/,,"CENTER",,,.F.)// "SALDO ATUAL"
    
 //	oSection1:Cell("CLANCDEB"):Hide()
 //	oSection1:Cell("CLANCCRD"):Hide()
 //	oSection1:Cell("CTPSLDATU"):Hide()
...
TRCell():New(oSection2,"CORRELATIVO"	,""       ,"LOTE/SUB/DOC/LINHA",/*Picture*/,If(nTamSegOfi < 20, 20,nTamSegOfi),/*lPixel*/,{|| cArqTmp->SEGOFI },/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// "LOTE/SUB/DOC/LINHA"
...
oSection1_1 := TRSection():New(oReport,"Total  ",/*{"cArqTmp","CT2"}*/,/*aOrder*/,/*lLoadCells*/,/*lLoadOrder*/) //STR0050
...
If oReport:Title() == oReport:cRealTitle //If Type("NewHead")== "U" 
	IF lAnalitico
		cTitulo	:=Iif(cPaisLoc=="PER","RAZAO ANALITICO EM ",	"RAZAO ANALITICO EM ")	//"RAZAO ANALITICO EM "  
		Titulo:=cTitulo
	Else
		cTitulo	:=	Iif(cPaisLoc=="PER","RAZAO SINTETICO EM ",	"RAZAO SINTETICO EM ")	//"RAZAO SINTETICO EM "     
		Titulo  := cTitulo+	cDescMoeda + " DE " + DTOC(dDataIni) +;	// "DE"
...
If mv_par39 = 2 
	oBrkConta 	:= TRBreak():New( oSection2, { || cArqTmp->NIT }, OemToAnsi("Total NIT"), )
...
#IFDEF TOP
If TcSrvType() != "AS/402" .And. cTipo == "1" .And. FunName() == 'CTBR402' .And. TCGetDb() $ "MSSQL7/MSSQL"		
	DEFAULT cUFilter	:= ".T."		
Else
#ENDIF
...
#IFDEF TOP
	If TcSrvType() != "AS/402" .And. cTipo == "1" .And. FunName() == 'CTBR402' .And. TCGetDb() $ "MSSQL7/MSSQL"		
		U_CtbQryRNIT(oMeter,oText,oDlg,lEnd,cContaIni,cContaFim,cCustoIni,cCustoFim,;
				cItemIni,cItemFim,cNitIni,cNitFim,cMoeda,dDataIni,dDataFim,;
		...
		While CT1->(!Eof()) .and. CT1->CT1_FILIAL == cFilCT1 .And. CT1->CT1_CONTA <= cContaFim
			...
			While !Eof() .And. CT2->CT2_FILIAL == cFilCT2 .And. CT2->CT2_DEBITO == CT1->CT1_CONTA .and. CT2->CT2_DATA <= dDataFim
				...
				If (CT2->CT2_DC == "1" .Or. CT2->CT2_DC == "3") .And. &(cValid) .And. &(cVldEnt) .And. CT2->CT2_TPSALD == cSaldo
					CT2->(CtbGrvRNIT(lJunta,cMoeda,cSaldo,"1",c2Moeda,cAliasCT2,nTipo))
				Endif
				dbSelectArea("CT2")
				dbSkip()
			EndDo
			...
			While !Eof() .And. CT2->CT2_FILIAL == cFilCT2 .And. CT2->CT2_CREDIT == CT1->CT1_CONTA .and. CT2->CT2_DATA <= dDataFim
				...
				If (CT2->CT2_DC == "2" .Or. CT2->CT2_DC == "3") .And. &(cValid) .And. &(cVldEnt) .And. CT2->CT2_TPSALD == cSaldo
					...
					Else
						...
						EndIf			
					EndIf			
					CT2->(CtbGrvRNIT(lJunta,cMoeda,cSaldo,"2",c2Moeda,cAliasCT2,nTipo))
				Endif
				dbSelectArea("CT2")
				dbSkip()
			EndDo			
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function XCtbGrvN(cConteudo,dDataL,cCpoTmp)
...
If cCpoTmp = "CONTA"
	Replace HISTORICO		With "CUENTA SEN MOVIMIENTO EN EL PERIODO"		//"CONTA SEM MOVIMENTO NO PERIODO"
ElseIf cCpoTmp = "CCUSTO"
	Replace HISTORICO		With Upper(AllTrim(CtbSayApro("CTT"))) + " "  + "SEN MOVIMIENTO EN EL PERIODO"	//"SEM MOVIMENTO NO PERIODO"
ElseIf cCpoTmp = "ITEM"
	Replace HISTORICO		With Upper(AllTrim(CtbSayApro("CTD"))) + " "  + "SEN MOVIMIENTO EN EL PERIODO"	//"SEM MOVIMENTO NO PERIODO"
ElseIf cCpoTmp = "NIT"
	Replace HISTORICO		With Upper(AllTrim("NIT")) + " "  + "SEN MOVIMIENTO EN EL PERIODO"	//"SEM MOVIMENTO NO PERIODO"
Endif
		...
		If lNoMovim
			...
			ElseIf lSldAnt 
				If SaldoCT7Fil(cArqCT2->CONTA,dDataIni,cMoeda,cSaldo,'CTBR402')[6] <> 0 .and. cArqTMP->CONTA <> cArqCT2->CONTA																							
					U_XCtbGrvN(cArqCT2->CONTA,dDataIni,"CONTA")	
				Endif			
			EndIf
		Else
			RecLock("cArqTmp",.T.)		    	
